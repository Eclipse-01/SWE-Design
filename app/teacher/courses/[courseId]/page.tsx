                  ...
                  assignments.map((assignment) => (
                    <div key={assignment.id} className="border rounded-lg p-4 space-y-3">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            截止: {format(new Date(assignment.deadline), 'MM-dd HH:mm', { locale: zhCN })}
                          </div>
                        </div>
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          assignment.status === 'PUBLISHED' ? 'bg-green-100 text-green-800' :
                      ...
                      <div className="flex justify-between items-center text-sm">
                        <div>
                          <span className="text-muted-foreground">提交:</span> {assignment._count.submissions}
                        </div>
                        <div>
                          <span className="text-muted-foreground">满分:</span> {assignment.maxScore}
                        </div>
                      </div>
                  ...
                  enrollments.map((enrollment) => (
                    <div key={enrollment.id} className="border rounded-lg p-4 flex justify-between items-center">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {format(new Date(enrollment.joinedAt), 'MM-dd', { locale: zhCN })}
                      </div>
                    </div>
                  ))
                )}
